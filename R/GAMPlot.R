GAMPlot <- function(data.frame){
  ...
  papers.comm1 <- papers.comm2 <- papers.comm3 <- rep(NA, 35)
  ...
  for(i in 1:35){
    ...
    papers.comm1[i] <- dim(comm1.byyear[[i]])[1]
    ...
    papers.comm2[i] <- dim(comm2.byyear[[i]])[1]
    ...
    papers.comm3[i] <- dim(comm3.byyear[[i]])[1]
  }
  ...
  lead.papers <- c(papers.comm1, papers.comm2, papers.comm3)
...
#   comm1.leadgam.2 <- gam(shannon.comm1 ~ s(lead.yr) + s(papers.comm1))
#   comm2.leadgam.2 <- gam(shannon.comm2 ~ s(lead.yr) + papers.comm2)
#   comm3.leadgam.2 <- gam(shannon.comm3 ~ s(lead.yr) + papers.comm3)
...
# 
#   leadpreds.comm1.2 <- predict(comm1.leadgam.2, type = "response", se.fit = TRUE)
#   leadfit.comm1.2 <- leadpreds.comm1.2$fit 
#   leadfit.up95.comm1.2 <-leadfit.comm1.2 - 1.96*leadpreds.comm1.2$se.fit 
#   leadfit.low95.comm1.2 <-leadfit.comm1.2 + 1.96*leadpreds.comm1.2$se.fit
#   
#   leadpreds.comm2.2 <- predict(comm2.leadgam.2, type="response", se.fit = TRUE)
#   leadfit.comm2.2 <- leadpreds.comm2.2$fit
#   leadfit.up95.comm2.2 <- leadfit.comm2.2 - 1.96*leadpreds.comm2.2$se.fit
#   leadfit.low95.comm2.2 <- leadfit.comm2.2 + 1.96*leadpreds.comm2.2$se.fit
#   
#   leadpreds.comm3.2 <- predict(comm3.leadgam.2, type="response", se.fit = TRUE)
#   leadfit.comm3.2 <- leadpreds.comm3.2$fit
#   leadfit.up95.comm3.2 <-leadfit.comm3.2 - 1.96*leadpreds.comm3.2$se.fit
#   leadfit.low95.comm3.2 <-leadfit.comm3.2 + 1.96*leadpreds.comm3.2$se.fit
...
#                     factor(community))
  ...
  par(mfrow = c(1, 3), mar = c(2, 6, 1, 1), oma = c(1, 1, 1, 1))
...
#        lwd = c(2, 2, 2),
#        bty = "n")
# mtext("(a)", side = 3, line = .5, at = 1990)  

# plot(shannon.comm1[- (1:10)] ~ seq(1990, 2014), 
#        ylab = "Lead author affiliation diversity", xlab = "Year",
#        ylim = c(-1, 3), pch = 16, col = "gold", type = "p", lwd = 2, 
#        xlim = c(1995, 2015))
#   lines(leadfit.low95.comm1.2 ~ lead.yr)
#   lines(leadfit.up95.comm1.2 ~ lead.yr)
# polygon(c(lead.yr, rev(lead.yr)), 
#           c(leadfit.low95.comm1.2, rev(leadfit.up95.comm1.2)),
# #          c(leadfit.low95.comm1.2, rev(leadfit.up95.comm1.2)),
#           border=NA, col = rgb(255/255,215/255,0, alpha = .3))
#   polygon(c(lead.yr, rev(lead.yr)), 
#           c(leadfit.low95.comm2.2, rev(leadfit.up95.comm2.2)), col = rgb(1,0,0, alpha = .3),
#           border=NA)
#   polygon(c(lead.yr, rev(lead.yr)), 
#           c(leadfit.low95.comm3.2, rev(leadfit.up95.comm3.2)), col = rgb(0,0,1, alpha = .3),
#           border=NA)
 
#svg("./Figures/Figure4_PNASSpec_20151017.svg", width = 8, heigh = 4)
par(mfrow = c(1, 2), oma = c(0, 0, 0, 1), mar = c(2, 4, 2, 0))
plot(shannon.comm1[- (1:10)] ~ seq(1990, 2014), 
     ylab = "Lead author affiliation diversity", xlab = "Year",
     ylim = c(-1, 3), pch = 16, col = "gold", type = "p", lwd = 2, 
     xlim = c(1995, 2015))
#lines(leadfit.low95.comm1 ~ lead.yr)
#lines(leadfit.up95.comm1 ~ lead.yr)
polygon(c(lead.yr, rev(lead.yr)), 
        c(leadfit.low95.comm1, rev(leadfit.up95.comm1)),
        #          c(leadfit.low95.comm1.2, rev(leadfit.up95.comm1.2)),
        col = rgb(255/255,215/255,0, alpha = .3), border="gold")
polygon(c(lead.yr, rev(lead.yr)), 
...
polygon(c(years, rev(years)), 
        ...
        border="gold", col = rgb(255/255,215/255,0, alpha = .3))

AuthorDiversityPlots <- function(data.frame){
  ...
  LeadDivCommonTrajectory.gam <- gam(lead.shannon ~ s(lead.year) + 
                       factor(lead.community))
  LeadDivPaperOffset.gam <- gam(lead.shannon ~ s(lead.papers) + s(lead.year) + 
                       s(lead.year, by = factor(lead.community)) +
                       factor(lead.community))
  AIC(LeadDiv.fit)
  AIC(LeadDiv.gam)
  ...
  comm1.leadgam2 <- gam(shannon.comm1 ~ s(lead.yr) + papers.comm1)
  comm2.leadgam2 <- gam(shannon.comm2 ~ s(lead.yr) + papers.comm2)
  comm3.leadgam2 <- gam(shannon.comm3 ~ s(lead.yr) + papers.comm3)
...
#      ylim = c(0, 3.5), pch = 16, col = "gold", xlim = c(1994, 2015))
  ...
  WithinGamFit <- gam(AuShannonDiv ~ s(PubYear) + 
                        s(PubYear, by = factor(Journal.Community)) +
                        factor(Journal.Community), data = data.frame.no4)
  summary(WithinGamFit)


par(mfrow = c(1, 1), mar = c(2, 6, 1, 1), oma = c(1, 1, 1, 1), cex.axis = 1.2, cex.lab = 1.2)
...
# Paul's check on number of authors vs. shannon diversity
data.frame.sm <- subset(data.frame, NumberAuthors != 1 & NumberAuthors != 0 & NumberAuthors <= 20)
par(mfrow = c(1, 2), oma = c(0, 0, 0, 0), mar = c(4, 4, 1, 1))
plot(data.frame.sm$AuShannonDiv ~ data.frame.sm$NumberAuthors, 
     xlab = "Number Authors",
     ylab = "Within-paper Author Diversity")
lines(lowess(data.frame.sm$AuShannonDiv ~ data.frame.sm$NumberAuthors))
plot(lead.shannon ~ lead.papers, 
     xlab = "Number Papers",
     ylab = "Lead Author Diversity")
lines(lowess(lead.shannon ~ lead.papers))
